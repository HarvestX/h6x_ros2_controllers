...
from h6x_controller_manager import configure_controller, list_controllers, \
...
def wait_for_controller_manager(node, h6x_controller_manager, timeout_duration):
    # List of service names from controller_manager we wait for
    service_names = (
        f'{h6x_controller_manager}/configure_controller',
        f'{h6x_controller_manager}/list_controllers',
        f'{h6x_controller_manager}/list_controller_types',
        f'{h6x_controller_manager}/list_hardware_components',
        f'{h6x_controller_manager}/list_hardware_interfaces',
        f'{h6x_controller_manager}/load_controller',
        f'{h6x_controller_manager}/reload_controller_libraries',
        f'{h6x_controller_manager}/switch_controller',
        f'{h6x_controller_manager}/unload_controller'
    )
    ...
    node_and_namespace = wait_for_value_or(
        lambda: find_node_and_namespace(node, h6x_controller_manager),
        node, timeout, None, f'\'{h6x_controller_manager}\' node to exist')
    ...
    if node_and_namespace:
        ...
        return wait_for_value_or(
            ...
            node, timeout, False, f"'{h6x_controller_manager}' services to be available")

    return False


def is_controller_loaded(node, h6x_controller_manager, controller_name):
    controllers = list_controllers(node, h6x_controller_manager).controller
...
def main(args=None):
    ...
    parser.add_argument(
        '-c', '--h6x-controller-manager', help='Name of the controller manager ROS node',
        default='h6x_controller_manager', required=False)
    parser.add_argument(
        '-p', '--param-file',
    ...
    controller_manager_name = args.h6x_controller_manager
